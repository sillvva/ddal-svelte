...
<svelte:boundary>
	...
	{@const isMac = request.current?.isMac}

	{#snippet pending()}
		<span class="hover-hover:md:input hover-hover:md:gap-4 hover-hover:md:cursor-text touch-hitbox flex h-10" aria-label="Search">
			{@render trigger("")}
		</span>
	{/snippet}
	...
	<Dialog.Root bind:open={() => open, setOpen}>
		<Dialog.Trigger
			...
			aria-label="Search"
		>
			{@render trigger(isMac === undefined ? "" : isMac ? "⌘" : "CTRL")}
		</Dialog.Trigger>
		<Dialog.Portal>
