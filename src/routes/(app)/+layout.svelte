<script lang="ts">
	...
	import { modal, showBg } from "$src/lib/store";
...
<div class="relative flex min-h-screen flex-col">
	<header
		class={twMerge(
			...
			(data.mobile || !$showBg) && "sticky top-0 border-slate-300 bg-base-300 dark:border-slate-700"
		)}
	>
		<nav class="container mx-auto flex max-w-5xl gap-2 p-4">
			...
			<a
				...
				class={twMerge(
					...
					(data.mobile || !$showBg) && "mr-2 flex-1 sm:flex-none md:mr-8"
				)}
			>
			...
			{/if}
			<div class={twMerge("flex-1", (data.mobile || !$showBg) && "hidden sm:block")}>&nbsp;</div>
			<a
						...
						<div class="avatar">
							<div
								class={twMerge(
									...
									(data.mobile || !$showBg) && "w-9 lg:w-11"
								)}
							>
								<img
